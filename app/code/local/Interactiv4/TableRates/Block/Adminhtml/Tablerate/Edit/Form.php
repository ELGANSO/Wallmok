<?php
class Interactiv4_TableRates_Block_Adminhtml_Tablerate_Edit_Form extends Mage_Adminhtml_Block_Widget_Form {
    /**
     * @var array
     */
    ...
    protected function _prepareForm() {
        ...
        $fieldset->addField('shipping_method_enabled', 'select', array(
            ...
            'required' => true,
            'value' => ($model->getMappedData('price_percentage') >= 0 ? '1' : '0'),
        ...
        if ($this->_getHelper()->supportsMarkup()) {
            $fieldset->addField('markup', 'select', array(
                'name' => 'markup',
                'label' => $this->_getHelper()->__('Shipping Price Calculation'),
                'required' => true,
                'value' => $model->getMappedData('markup'),
                'values' => array('0' => $this->_getHelper()->__("Fixed Price"), '1' => $this->_getHelper()->__('Add Percentage')),
        ...
        $fieldset->addField('price_percentage', 'text', array(
            ...
            'required' => true,
            'value' => $model->getMappedData('price_percentage'),
            'class' => 'validate-number'
        ));
        if ($this->_getHelper()->supportsCashOnDelivery()) {
            ...
            $fieldset->addField('cod_option', 'select', array(
                ...
                'value' => $codOption,
                'values' => array(
                    Interactiv4_TableRates_Model_Tablerate::COD_NOT_AVAILABLE => $this->_getHelper()->__("Cash On Delivery Not Available"),
                    Interactiv4_TableRates_Model_Tablerate::COD_SURCHARGE_ZERO => $this->_getHelper()->__("Zero Surcharge"),
                    Interactiv4_TableRates_Model_Tablerate::COD_SURCHARGE_FIXED => $this->_getHelper()->__('Fixed Surcharge'),
                    Interactiv4_TableRates_Model_Tablerate::COD_SURCHARGE_PERCENTAGE => $this->_getHelper()->__('Percentage Surcharge')
                ),
            ));
            $fieldset->addField('cashondelivery_surcharge', 'text', array(
                ...
                'required' => true,
                'value' => $codOption == Interactiv4_TableRates_Model_Tablerate::COD_SURCHARGE_PERCENTAGE ? $model->getData('cod_surcharge_percentage') : $model->getData('cod_surcharge_price'),
                'class' => 'validate-number',
            ));
    ...
    /**
     *
     * @return \Interactiv4_TableRates_Block_Adminhtml_Tablerate_Edit_Form 
     */
