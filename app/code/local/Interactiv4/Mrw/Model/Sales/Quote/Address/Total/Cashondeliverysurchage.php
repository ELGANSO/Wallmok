<?php
class Interactiv4_Mrw_Model_Sales_Quote_Address_Total_Cashondeliverysurchage extends Mage_Sales_Model_Quote_Address_Total_Abstract {
    public function __construct() {
        $this->setCode('i4mrwes_cashondelivery_surcharge');
    }
    public function collect(Mage_Sales_Model_Quote_Address $address) {
        $address->setData('i4mrwes_cashondelivery_surcharge', 0);
        $address->setData('base_i4mrwes_cashondelivery_surcharge', 0);
        $address->setData('i4mrwes_cashondelivery_surcharge_tax', 0);
        $address->setData('base_i4mrwes_cashondelivery_surcharge_tax', 0);        
        $paymentMethod = $address->getQuote()->getPayment()->getMethod();
        if ($paymentMethod == 'i4mrwes_cashondelivery' && $address->getAddressType() == 'shipping') {
            ...
            $address->setData('i4mrwes_cashondelivery_surcharge', $cashondeliverySurcharge);
            $address->setData('base_i4mrwes_cashondelivery_surcharge', $baseCashondeliverySurcharge);
            $this->_calculateSurchargeSalesTax($address);   
            $quote->save();
        }
        $address->setGrandTotal($address->getGrandTotal() + $address->getData('i4mrwes_cashondelivery_surcharge'));
        $address->setBaseGrandTotal($address->getBaseGrandTotal() + $address->getData('base_i4mrwes_cashondelivery_surcharge'));
        return $this;
    }
     ...
     * @return \Interactiv4_Mrw_Model_Sales_Quote_Address_Total_Cashondeliverysurchage 
     */
    protected function _calculateSurchargeSalesTax(Mage_Sales_Model_Quote_Address $address) {
        
        $taxCalculator = Mage::getSingleton('tax/calculation'); /* @var $taxCalculator Mage_Tax_Model_Calculation */
        ...
        $surcharge      = $address->getData('i4mrwes_cashondelivery_surcharge');
        $baseSurcharge  = $address->getData('base_i4mrwes_cashondelivery_surcharge');
        
        // NOTA: Mira el comentario de 25 abr 2013 10:45 en #43 de SEUR collab.
        ...
        $address->setData('i4mrwes_cashondelivery_surcharge_tax', $surchargeTax);
        $address->setData('base_i4mrwes_cashondelivery_surcharge_tax', $baseSurchargeTax);
        
        if ($inclTax) {
            $address->setData('i4mrwes_cashondelivery_surcharge', $surcharge - $surchargeTax);
            $address->setData('base_i4mrwes_cashondelivery_surcharge', $baseSurcharge - $baseSurchargeTax);            
        }
        return $this;
    }    
    ...
    public function fetch(Mage_Sales_Model_Quote_Address $address) {
        $amount = $address->getData('i4mrwes_cashondelivery_surcharge');
        if ($amount != 0 && $address->getAddressType() == 'shipping') {
            $title = Mage::getStoreConfig('payment/i4mrwes_cashondelivery/total_title',  $address->getQuote()->getStore());
    ...
    public function getLabel() {
        $title = Mage::getStoreConfig('payment/i4mrwes_cashondelivery/total_title');
        return $title;
    }
